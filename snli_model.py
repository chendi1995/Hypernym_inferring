...
class CustomModel:
    def __init__(self, XMAXLEN, YMAXLEN, embedding_size):
    ...
    def build_model(self):
        with tf.variable_scope("input_layer"):
            ...
            self.label = tf.placeholder("float", [None, 3])
        ...
        with tf.variable_scope("concact_layer"):
            ...
            output = tf.concat([self.x_att,temp2,temp1,y_att], 1)


        with tf.variable_scope("dense_layer"):
            output = tf.layers.dense(inputs=output, units=3, activation=None)

            self.logits = tf.nn.dropout(output, self.drop_rate)
        ...
        with tf.variable_scope("optimizer"):
            self.optimizer = tf.train.RMSPropOptimizer(self.learning_rate, 0.9).minimize(self.cross_entropy)
