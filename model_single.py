...
class CustomModel:
    ...
    def build_model(self):
        ...
        with tf.variable_scope("encode_x"):
            ...
            x_outputs, x_state = tf.nn.dynamic_rnn(cell=fw_lstm,inputs=self.X, dtype=tf.float32)
        with tf.variable_scope("encode_y"):
            fw_lstm = rnn.BasicLSTMCell(self.lstm_unit, state_is_tuple=True)
            fw_lstm = rnn.DropoutWrapper(fw_lstm, input_keep_prob=1.0, output_keep_prob=1.0, seed=None)

            y_outputs, y_state = tf.nn.dynamic_rnn(cell=fw_lstm,inputs=self.Y, dtype=tf.float32)
          
            # outputs = [batch_size,MAXLEN,embedding_size]



        with tf.variable_scope("inner_attention"):
            Xn = tf.reduce_mean(x_outputs, 1)
            Ubx = tf.Variable(tf.random_normal([self.lstm_unit, self.lstm_unit]), name='Ubx')
            bnx = tf.matmul(Xn, Ubx)
            mx = []
            Ucx = tf.Variable(tf.random_normal([self.lstm_unit, self.lstm_unit]), name='Ucx')
            Uax = tf.Variable(tf.random_normal([self.lstm_unit, 1]), name='Uax')
            ...
            self.x_att = tf.reshape(x_att, [-1, self.lstm_unit])

            # for i in range(self.batch_size):
            #     x_att.append(tf.transpose(tf.matmul(tf.transpose(x_outputs[i]), sx[i])))
            # x_att = tf.reshape(x_att, [self.batch_size, self.lstm_unit])

            Yn = tf.reduce_mean(y_outputs, 1)
            Uby = tf.Variable(tf.random_normal([self.lstm_unit, self.lstm_unit]), name='Uby')
            bny = tf.matmul(Yn, Uby)
            my = []
            Ucy = tf.Variable(tf.random_normal([self.lstm_unit, self.lstm_unit]), name='Ucy')
            Uay = tf.Variable(tf.random_normal([self.lstm_unit, 1]), name='Uay')
            ...
            y_att = tf.reshape(y_att, [-1, self.lstm_unit])

        with tf.variable_scope("concat_hyper_feature"):
            D_feature = tf.layers.dense(inputs=self.D, units=self.lstm_unit, activation=None)
